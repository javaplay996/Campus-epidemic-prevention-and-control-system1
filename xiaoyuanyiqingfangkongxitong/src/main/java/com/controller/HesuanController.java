...
public class HesuanController {
    ...
    private static final String TABLE_NAME = "hesuan";
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        params.put("hesuanDeleteStart",1);params.put("hesuanDeleteEnd",1);
    ...
    */
    ...
    public R save(@RequestBody HesuanEntity hesuan, HttpServletRequest request){
        ...
        Wrapper<HesuanEntity> queryWrapper = new EntityWrapper<HesuanEntity>()
            ...
            .eq("hesuan_name", hesuan.getHesuanName())
            .eq("hesuan_delete", 1)
            ;
    ...
    */
    ...
    public R update(@RequestBody HesuanEntity hesuan, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,hesuan:{}",this.getClass().getName(),hesuan.toString());
        HesuanEntity oldHesuanEntity = hesuanService.selectById(hesuan.getId());//查询原先数据
...
//        else if("用户".equals(role))
//            hesuan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(hesuan.getHesuanFile()) || "null".equals(hesuan.getHesuanFile())){
                hesuan.setHesuanFile(null);
        }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<HesuanEntity> oldHesuanList =hesuanService.selectBatchIds(Arrays.asList(ids));//要删除的数据
        ArrayList<HesuanEntity> list = new ArrayList<>();
        for(Integer id:ids){
            HesuanEntity hesuanEntity = new HesuanEntity();
            hesuanEntity.setId(id);
            hesuanEntity.setHesuanDelete(2);
            list.add(hesuanEntity);
        }
        if(list != null && list.size() >0){
            hesuanService.updateBatchById(list);
        }
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            HesuanEntity hesuanEntity = new HesuanEntity();
//                            hesuanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            hesuanEntity.setHesuanName(data.get(0));                    //核酸打卡 要改的
//                            hesuanEntity.setHesuanFile(data.get(0));                    //核酸照片 要改的
//                            hesuanEntity.setHesuanText(data.get(0));                    //备注 要改的
//                            hesuanEntity.setHesuanDelete(1);//逻辑删除字段
...
//                            hesuanEntity.setCreateTime(date);//时间
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody HesuanEntity hesuan, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,hesuan:{}",this.getClass().getName(),hesuan.toString());
        Wrapper<HesuanEntity> queryWrapper = new EntityWrapper<HesuanEntity>()
            .eq("yonghu_id", hesuan.getYonghuId())
            .eq("hesuan_name", hesuan.getHesuanName())
            .eq("hesuan_text", hesuan.getHesuanText())
            .eq("hesuan_delete", hesuan.getHesuanDelete())
//            .notIn("hesuan_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        HesuanEntity hesuanEntity = hesuanService.selectOne(queryWrapper);
        if(hesuanEntity==null){
