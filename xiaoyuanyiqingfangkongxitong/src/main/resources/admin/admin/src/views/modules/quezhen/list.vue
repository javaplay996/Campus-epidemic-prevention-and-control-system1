        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '确诊打卡' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.quezhenName" placeholder="确诊打卡" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '是否确诊' : ''">
                        <el-select v-model="searchForm.quezhenTypes" placeholder="请选择是否确诊">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in quezhenTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('quezhen','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingfangkongxitong/upload/quezhenMuBan.xls"
                        >批量导入确诊上报数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('quezhen','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="quezhenUploadSuccess"
                                :on-error="quezhenUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('quezhen','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入确诊上报数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('quezhen','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "quezhen.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="确诊打卡">
                        <template slot-scope="scope">
                            {{scope.row.quezhenName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="quezhenFile"
                                     header-align="center"
                                     width="200"
                                     label="确诊照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.quezhenFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.quezhenFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="备注">
                        <template slot-scope="scope">
                            <span v-if="scope.row.quezhenText != null &&scope.row.quezhenText.length>10">
                                {{scope.row.quezhenText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.quezhenText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('quezhen','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('quezhen','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('quezhen','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'quezhen',//当前表表名,
                        // sumColum : 'quezhen_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'quezhen_name',//分组字符串字段
                        types : 'quezhen_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.quezhenName!= '' && this.searchForm.quezhenName!= undefined) {
                    params['quezhenName'] = '%' + this.searchForm.quezhenName + '%'
                }
                 
                if (this.searchForm.quezhenTypes!= '' && this.searchForm.quezhenTypes!= undefined) {
                    params['quezhenTypes'] = this.searchForm.quezhenTypes
                }
                                
                params['quezhenDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=quezhen_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.quezhenTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            quezhenUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "quezhen/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
