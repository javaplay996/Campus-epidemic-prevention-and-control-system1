        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '到校' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.daoxiaoName" placeholder="到校" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('daoxiao','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingfangkongxitong/upload/daoxiaoMuBan.xls"
                        >批量导入到校确认数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('daoxiao','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="daoxiaoUploadSuccess"
                                :on-error="daoxiaoUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('daoxiao','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入到校确认数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('daoxiao','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "daoxiao.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="到校">
                        <template slot-scope="scope">
                            {{scope.row.daoxiaoName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="daoxiaoFile"
                                     header-align="center"
                                     width="200"
                                     label="照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.daoxiaoFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.daoxiaoFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="备注">
                        <template slot-scope="scope">
                            <span v-if="scope.row.daoxiaoText != null &&scope.row.daoxiaoText.length>10">
                                {{scope.row.daoxiaoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.daoxiaoText}}
                            </span>
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="insertShijianTime"
                                   header-align="center"
                                   label="到校时间">
                        <template slot-scope="scope">
                            {{scope.row.insertShijianTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('daoxiao','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('daoxiao','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('daoxiao','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'daoxiao',//当前表表名,
                        sumColum : 'daoxiao_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'daoxiao_name',//分组字符串字段
                        // types : 'daoxiao_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.daoxiaoName!= '' && this.searchForm.daoxiaoName!= undefined) {
                    params['daoxiaoName'] = '%' + this.searchForm.daoxiaoName + '%'
                }
                                                
                params['daoxiaoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            daoxiaoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "daoxiao/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
